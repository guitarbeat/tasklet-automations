...
import {
  ...
  Circle, Hash, Timer, Inbox, TrendingUp
} from 'lucide-react';
...
import {
  ...
  resolutionInfo, RESOLUTIONS, PRIORITY
} from './utils/helpers';
...
export default function App() {
  ...
  const resolved = queue.filter(q => q.state === 'resolved' || q.state === 'failed' || q.state === 'skipped');
    ...
    <div className="min-h-screen bg-base-100">
      ...
      <header className="sticky top-0 z-40 bg-base-100/90 backdrop-blur-md border-b border-base-200">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wider ${
                activePR ? 'bg-info/15 text-info' : 'bg-base-200 text-base-content/35'
              }`}>
                <Circle size={6} className={activePR ? 'fill-current animate-pulse-soft' : 'fill-current'} />
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="join bg-base-200 rounded-lg p-0.5">
              <button className={`join-item btn btn-xs border-0 min-w-[4rem] ${view === 'session' ? 'btn-neutral' : 'btn-ghost'}`}
                onClick={() => setView('session')}>Session</button>
              <button className={`join-item btn btn-xs border-0 min-w-[4rem] ${view === 'overview' ? 'btn-neutral' : 'btn-ghost'}`}
                onClick={() => setView('overview')}>Overview</button>
            </div>
            <button className="btn btn-sm btn-primary gap-1.5 rounded-lg" onClick={agentRunScan} disabled={agentBusy}>
        ...
        <div className="max-w-3xl mx-auto px-4 pb-3 flex items-center gap-3 text-sm">
          ...
          <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg ${staleCount > 0 ? 'bg-warning/10' : 'bg-base-200/60'}`}>
            <AlertTriangle size={13} className={staleCount > 0 ? 'text-warning' : 'opacity-50'} />
            <span className={`font-bold tabular-nums ${staleCount > 0 ? 'text-warning' : ''}`}>{staleCount}</span>
          ...
          <div className="space-y-5 stagger">
                  ...
                  <React.Fragment key={step.key}>
                    {i > 0 && (
                      <div className={`flex-1 h-px mx-1 ${
                        isDone ? 'bg-success' : isActive ? 'step-active-line h-0.5' : 'bg-base-300'
                      }`} />
                    )}
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <span className={`text-[10px] leading-tight text-center max-w-[5rem] ${
            ...
            <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border transition-colors ${
              ...
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                nextAction.urgent ? 'bg-warning/15 text-warning' : 'bg-base-300/60 text-base-content/30'
              }`}>
                <ArrowRight size={15} />
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-bold leading-tight ${nextAction.urgent ? 'text-warning' : 'text-base-content/80'}`}>
                  Next Step: {nextAction.text}
                </p>
                <p className="text-xs text-base-content/50 mt-0.5 leading-tight">{nextAction.detail}</p>
              </div>
              {!activePR && queued.length > 0 && (
                <button className="btn btn-sm btn-info gap-1.5 rounded-lg shrink-0"
                  onClick={() => agentActivatePR(queued[0])} disabled={agentBusy}>
                  {agentBusy ? <span className="loading loading-spinner loading-xs" /> : <Play size={13} />}
            ...
            <section className="animate-fade-in">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="badge badge-sm bg-base-200 text-base-content/50 font-mono">{queued.length}</span>
                  <span className="text-[11px] text-base-content/40 hidden sm:inline">— PRs waiting for Jules rebase</span>
                </div>
                {unqueuedStaleCount > 0 && (
                  <button className="btn btn-xs gap-1 btn-outline btn-primary rounded-lg" onClick={queueAllStale}
              ...
              {queued.length === 0 ? (
                <div className="rounded-xl border border-dashed border-base-300 py-10 text-center">
                  ...
                  <p className="text-sm text-base-content/35 font-medium">Queue is empty — all caught up!</p>
                </div>
              ) : (
                <div className="space-y-1.5">
                  {queued.map((q, i) => {
                    ...
                    return (
                      <div key={q.id} className={`queue-item flex items-center gap-3 px-3.5 py-2.5 rounded-xl border transition-colors ${
                        isNext ? 'bg-info/5 border-info/20' : 'bg-base-200/40 border-transparent hover:border-base-300'
                      }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <a href={pr?.url || '#'} target="_blank" rel="noopener"
                            ...
                            <span className="text-base-content/30 font-mono text-xs">#{q.pr_number}</span>{' '}
                            {pr?.title || q.notes || ''}
                          </a>
                          <div className="flex items-center gap-1.5 mt-1">
                            ...
                            <span className={`inline-flex items-center gap-0.5 px-1.5 py-0 rounded text-[10px] font-medium border ${pMeta.color}`}>
                              {pMeta.icon} {pMeta.label}
                            </span>
                            {isNext && (
                              <span className="text-[10px] font-bold text-info uppercase tracking-wider">next →</span>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center gap-0.5 shrink-0">
                          ...
                          {canActivate && (
                            <button className="btn btn-xs btn-info gap-1 ml-1 rounded-lg"
                              onClick={() => agentActivatePR(q)} disabled={agentBusy}
                              title="Start a Jules rebase session for this PR">
                              {agentBusy ? <span className="loading loading-spinner loading-xs" /> : <Play size={11} />}
                              Activate
                            </button>
                          )}
                          <button 
              ...
              <section className="animate-fade-in">
                ...
                <div className="rounded-xl border border-base-200 overflow-hidden divide-y divide-base-200">
                  {activityLog.slice(0, 10).map(entry => (
                    <div key={entry.id} className="flex items-center gap-2.5 py-2 px-3 text-sm bg-base-100 hover:bg-base-200/30 transition-colors">
                      <span className="text-base-content/40">{ACTION_META[entry.action]?.icon || <Clock size={13} />}</span>
                      <span className="font-mono text-[11px] text-base-content/25">#{entry.pr_number}</span>
                      <span className="text-xs font-medium">{entry.action}</span>
                      {entry.details && <span className="text-xs text-base-content/35 truncate flex-1">— {entry.details}</span>}
            ...
            {/* ━━━ History ━━━ */}
            {resolved.length > 0 && (
              <HistorySection items={resolved} onRemove={removeFromQueue} />
            )}
          </div>
        ) : (
          /* ━━━━ OVERVIEW TAB ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
          <div className="space-y-5 stagger">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {repos.map(r => {
                ...
                return (
                  <div key={r.name} className="animate-fade-in rounded-xl border border-base-200 p-4 card-hover">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                      ...
                      <div className={`text-xl font-bold tabular-nums ${
                        ...
                        {pct}<span className="text-sm font-normal text-base-content/25">%</span>
                      </div>
                    </div>
                    <div className="flex h-2 rounded-full overflow-hidden bg-base-200 gap-px">
                      {r.current > 0 && (
                        <div className="bg-success rounded-full transition-all duration-700"
                          style={{ width: `${(r.current / r.total) * 100}%` }} />
                      )}
                      {r.stale > 0 && (
                        <div className="bg-warning rounded-full transition-all duration-700"
                          style={{ width: `${(r.stale / r.total) * 100}%` }} />
                      )}
                      {r.dead > 0 && (
                        <div className="bg-error rounded-full transition-all duration-700"
                          style={{ width: `${(r.dead / r.total) * 100}%` }} />
                      )}
                    </div>
                    <div className="flex gap-4 mt-2.5 text-[11px] text-base-content/40">
                      <span className="flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-success" />{r.current} current
                      </span>
                      <span className="flex items-center gap-1.5">
                        <span className="w-1.5 h-1.5 rounded-full bg-warning" />{r.stale} stale
                      </span>
                      {r.dead > 0 && (
                        <span className="flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-error" />{r.dead} dead
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {([
                { label: 'Pipeline',  value: activePR ? 'Active' : 'Idle', icon: <Activity size={14} />, cls: activePR ? 'text-info' : 'text-base-content/30' },
                { label: 'Queued',    value: String(queued.length),   icon: <Inbox size={14} />,      cls: queued.length > 0 ? 'text-warning' : 'text-base-content/30' },
                { label: 'Resolved',  value: String(resolved.length), icon: <CheckCircle size={14} />, cls: 'text-success' },
                { label: 'Stale',     value: String(staleCount),      icon: <AlertTriangle size={14} />, cls: staleCount > 0 ? 'text-warning' : 'text-success' },
              ] as const).map(s => (
                <div key={s.label} className="rounded-xl border border-base-200 p-3 text-center card-hover">
              ...
              <div className="flex gap-1 mb-3">
                {([
                  ...
                  <button key={f.key}
                    ...
                    {f.label}
                    <span className={`text-[10px] tabular-nums ${filter === f.key ? 'opacity-70' : 'opacity-40'}`}>
                      {f.count}
                    </span>
                  </button>
                ))}
                    ...
                    <div key={`${pr.repo}-${pr.number}`}
                      ...
                      {qItem ? (
                        <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium ${
                          ...
                          qItem.state === 'resolved' ? 'bg-success/15 text-success' :
                        ...
                        <div className="flex gap-1 shrink-0">
                          <button className="btn btn-xs btn-ghost text-primary rounded-lg" onClick={() => queuePR(pr)}
                            disabled={actionLoading === `q-${pr.number}`}>Queue</button>
                          {isDead && (
                            <button className="btn btn-xs btn-ghost text-error rounded-lg" onClick={() => agentAutoClose(pr)}
...
const HistorySection: React.FC<{
  items: QueueItem[]; onRemove: (item: QueueItem) => void;
}> = ({ items, onRemove }) => {
      ...
      {open && (
        <div className="rounded-xl border border-base-200 overflow-hidden divide-y divide-base-200/80 mt-3">
          {items.map(q => {
            ...
            return (
              <div key={q.id} className="animate-fade-in flex items-center gap-2.5 px-3 py-2 text-sm hover:bg-base-200/30 transition-colors">
