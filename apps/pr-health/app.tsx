...
import {
  RefreshCw, Radar, ChevronUp, ChevronDown, X, Play, Search,
  CheckCircle, XCircle, SkipForward, Bell, GitMerge, RotateCcw,
  Clock, ArrowRight, Archive, ChevronRight, ExternalLink,
  AlertTriangle, Zap, Shield, Palette, Package, Activity,
  Circle, Hash, Timer, Inbox, TrendingUp, Trash2, CalendarDays,
  FileCode, Eye, ThumbsUp, ThumbsDown, MessageSquare, Gauge, Info,
  FolderGit2, Lock, Star, Check, KeyRound, ShieldCheck, ShieldAlert
} from 'lucide-react';
import { PullRequest, QueueItem, RepoHealth, ActivityLogEntry, HealthScore, PRDetail, TokenStatus } from './types';
import { ConversationPanel } from './components/ConversationPanel';
import { TokenPanel } from './components/TokenPanel';
import { JulesAuditPanel } from './components/JulesAuditPanel';
import { PRTreemap } from './components/PRTreemap';
import {
  daysSince, formatAge, detectAgent, AGENT, staleness, repoHealth,
...
const PRIORITY_META: Record<number, { label: string; icon: React.ReactNode; color: string }> = {
  3: { label: 'Critical', icon: <Shield size={11} />, color: 'text-error bg-error/10 border-error/20' },
  2: { label: 'High',     icon: <Zap size={11} />,    color: 'text-warning bg-warning/10 border-warning/20' },
  1: { label: 'Normal',   icon: <Palette size={11} />, color: 'text-info bg-info/10 border-info/20' },
  0: { label: 'Low',      icon: <Package size={11} />, color: 'text-base-content/40 bg-base-200 border-base-300' },
};
...
const ACTION_META: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
  ...
  'status-check':   { icon: <Search size={13} />,      label: 'Status checked',         color: 'text-base-content/60' },
  resolved:         { icon: <CheckCircle size={13} />, label: 'Resolved',               color: 'text-success' },
  resolved_merged:  { icon: <GitMerge size={13} />,    label: 'Merged successfully',    color: 'text-success' },
  ...
  'auto-closed':    { icon: <Archive size={13} />,     label: 'Auto-closed (dead PR)',  color: 'text-warning' },
  scanned:          { icon: <Radar size={13} />,       label: 'Scan completed',         color: 'text-primary' },
};
const defaultActionMeta = { icon: <Clock size={13} />, label: 'Action', color: 'text-base-content/40' };
...
export default function App() {
  ...
  /* ─── API cost tracking ─────────────────────────────────────── */
  const [apiCalls, setApiCalls] = useState<{ listPR: number; getPR: number; sql: number; total: number; costCents: number }>({
    listPR: 0, getPR: 0, sql: 0, total: 0, costCents: 0,
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.all(selectedRepos.map(async (repo) => {
        const [owner, name] = repo.split('/');
        calls.listPR++;
        return { repo, data: await api.runTool('conn_8et0d5bx3yszdanafpnb__github_list_pull_requests', {
      ...
      for (const { repo, data } of results) {
        ...
        for (const pr of items as any[]) {
          allPrs.push({
            number: pr.number, title: pr.title, repo,
            author: pr.user || 'unknown', createdAt: pr.createdAt, updatedAt: pr.updatedAt,
            baseSha: pr.base?.sha || '', headBranch: pr.head?.ref || '',
      ...
      const fetchDetail = async (pr: PullRequest): Promise<{ number: number; repo: string; detail: PRDetail }> => {
        try {
          ...
          return { number: pr.number, repo: pr.repo, detail: {
            draft: false, labels: [], additions: 0, deletions: 0, changedFiles: 0,
            mergedAt: null, closedAt: null, reviewStatus: 'NO_REVIEWS',
            approvedBy: [], changesRequestedBy: [], totalReviews: 0,
          }};
        }
      };
      ...
      try {
        calls.sql++;
        const logRes = await api.sqlQuery("SELECT * FROM jules_activity_log ORDER BY created_at DESC LIMIT 50");
  ...
  useEffect(() => { if (reposReady) load(); }, [load, reposReady]);

  /* Load merge-token status (token-free metadata) once on mount. */
  useEffect(() => {
    (async () => {
      try {
        const raw = await api.readFileFromDisk('/tasklet/agent/home/.secrets/github-merge-token.status.json');
  ...
  const openRepoPicker = () => { setRepoPickerOpen(true); void fetchAvailableRepos(); };
  ...
  const withAction = async (key: string, fn: () => Promise<void>) => {
    ...
    try { await fn(); } catch (e: any) { showToast(e.message || 'Action failed', 'error'); }
    setActionLoading(null);
  };

  const queuePR = (pr: PullRequest) => withAction(`q-${pr.number}`, async () => {
    const priority = PRIORITY[pr.agentType] ?? 0;
    await api.sqlExec(`INSERT OR IGNORE INTO jules_queue (pr_number, repo, state, priority, notes) VALUES (${pr.number}, '${pr.repo}', 'queued', ${priority}, '${pr.title.replace(/'/g, "''")}')`);
  ...
  const failPR = (item: QueueItem) => withAction(`fail-${item.id}`, async () => {
    await api.sqlExec(`UPDATE jules_queue SET state = 'failed', resolution = 'jules-failed', resolved_at = datetime('now'), updated_at = datetime('now') WHERE id = ${item.id}`);
    showToast(`#${item.pr_number} marked failed`);
    await reloadQueue();
  });

  const skipPR = (item: QueueItem) => withAction(`skip-${item.id}`, async () => {
    await api.sqlExec(`UPDATE jules_queue SET state = 'skipped', resolved_at = datetime('now'), updated_at = datetime('now') WHERE id = ${item.id}`);
    showToast(`#${item.pr_number} skipped`);
    await reloadQueue();
  });

  const changePriority = (item: QueueItem, delta: number) => withAction(`pri-${item.id}`, async () => {
    const p = Math.max(0, Math.min(3, item.priority + delta));
    await api.sqlExec(`UPDATE jules_queue SET priority = ${p}, updated_at = datetime('now') WHERE id = ${item.id}`);
    await reloadQueue();
  });

  const queueAllStale = () => withAction('queue-all', async () => {
    const stalePrs = prs.filter(p => {
      const s = staleness(p, mainShas[p.repo]);
      return (s === 'stale' || s === 'dead') && !queue.some(q => q.pr_number === p.number && q.repo === p.repo);
    });
    let added = 0;
  ...
  const archiveItem = (item: QueueItem) => withAction(`arc-${item.id}`, async () => {
    await api.sqlExec(`UPDATE jules_queue SET state = 'archived', updated_at = datetime('now') WHERE id = ${item.id}`);
  ...
  const bulkArchive = (ids: number[]) => withAction('bulk-arc', async () => {
    await api.sqlExec(`UPDATE jules_queue SET state = 'archived', updated_at = datetime('now') WHERE id IN (${ids.join(',')})`);
  ...
  const queued = queue.filter(q => q.state === 'queued').sort((a, b) => b.priority - a.priority || a.id - b.id);
  const resolved = queue.filter(q => q.state === 'resolved' || q.state === 'failed' || q.state === 'skipped' || q.state === 'archived');
  ...
  const queuedKeys = useMemo(
    () => new Set(queue.filter(q => q.state === 'queued' || q.state === 'active').map(q => `${q.repo}#${q.pr_number}`)),
    [queue],
  );
  const activeLogEntries = activePR
    ? activityLog.filter(l => l.pr_number === activePR.pr_number && l.repo === activePR.repo)
    : [];
  const filtered = useMemo(() => prs.filter(p => filter === 'all' || staleness(p, mainShas[p.repo]) === filter)
    .sort((a, b) => {
  ...
  function nextActionText(item: QueueItem | undefined): { text: string; detail: string; urgent: boolean } {
    if (!item) return {
  ...
  return (
    <div className="min-h-screen bg-base-100">
      ...
      {/* ─ Repo Picker ─ */}
      {repoPickerOpen && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setRepoPickerOpen(false)}>
          <div className="bg-base-100 rounded-xl shadow-xl max-w-md w-full max-h-[80vh] flex flex-col animate-fade-in" onClick={(e) => e.stopPropagation()}>
            <div className="px-5 py-3 border-b border-base-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="badge badge-sm badge-primary badge-outline tabular-nums">{selectedRepos.length}</span>
              </div>
              <button className="btn btn-xs btn-ghost btn-square" onClick={() => setRepoPickerOpen(false)}>
                <X size={14} />
              </button>
            </div>
            <div className="px-5 py-2 border-b border-base-200/60">
              <p className="text-[11px] text-base-content/50 leading-snug">
                Pick which repos the dashboard and caretaker watch. Selection is saved and persists across sessions.
              </p>
            </div>
                        ...
                        <li key={r.fullName}>
                          ...
                          >
                            <span className={`flex items-center justify-center w-4 h-4 rounded border ${
                              active ? 'bg-primary border-primary text-primary-content' : 'border-base-300'
                            }`}>
                            ...
                            <span className="flex-1 min-w-0">
                              ...
                              <span className="flex items-center gap-2 text-[10px] text-base-content/40">
                                ...
                                {r.stargazersCount > 0 && (
                                  <span className="flex items-center gap-0.5"><Star size={8} />{r.stargazersCount}</span>
                                )}
                              </span>
                            </span>
                          </button>
            ...
            <div className="px-5 py-2.5 border-t border-base-200 flex items-center justify-between">
              <button className="btn btn-xs btn-ghost gap-1" onClick={() => persistRepos(DEFAULT_REPOS)} title="Reset to the two default repos">
                <RotateCcw size={11} /> Defaults
              </button>
              <button className="btn btn-xs btn-primary" onClick={() => setRepoPickerOpen(false)}>Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 glass-panel border-b border-base-200/50">
        ...
        <div className="max-w-3xl mx-auto px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h1 className="text-base font-extrabold tracking-tight gradient-text">Jules Pipeline</h1>
            <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
              activePR ? 'bg-info/15 text-info animate-glow' : 'bg-base-200/60 text-base-content/30'
            }`}>
              <Circle size={4} className={activePR ? 'fill-current animate-pulse-soft' : 'fill-current'} />
              {activePR ? 'Active' : 'Idle'}
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            <button className="btn btn-sm btn-primary gap-1 rounded-xl shadow-sm shadow-primary/20 h-7 min-h-0 text-xs" onClick={agentRunCaretaker} disabled={agentBusy} title="Run the full caretaker pass: scan PRs & issues, auto-merge safe bot PRs, sync the queue">
              {agentBusy ? <span className="loading loading-spinner loading-xs" /> : <Radar size={12} />}
              Run Caretaker
            </button>
            <button className="btn btn-sm btn-ghost gap-1 rounded-xl hover:bg-base-200/60 h-7 min-h-0 text-xs" onClick={openRepoPicker} title="Choose which repositories to monitor">
              <FolderGit2 size={12} />
            ...
            <button
              ...
              title={tokenStatus.configured ? `Merge token active (as ${tokenStatus.login})` : 'No merge token — merges & sync are blocked. Click to paste one.'}
            >
              {tokenStatus.configured ? <ShieldCheck size={12} /> : <ShieldAlert size={12} />}
              <span className="hidden sm:inline">Token</span>
            </button>
            <button className="btn btn-sm btn-ghost btn-square rounded-xl hover:bg-base-200/60 h-7 min-h-0 w-7" onClick={load} title="Refresh data">
        ...
        <div className="max-w-3xl mx-auto px-4 pb-2 flex items-center gap-1.5 flex-wrap text-[11px]">
          ...
          {(() => {
            const scores = prs.filter(p => p.detail).map(p => computeHealthScore(p, mainShas[p.repo]));
            const avg = scores.length ? Math.round(scores.reduce((a, s) => a + s.score, 0) / scores.length) : 0;
            const avgGrade = avg >= 90 ? 'A' : avg >= 70 ? 'B' : avg >= 50 ? 'C' : avg >= 30 ? 'D' : 'F';
            return scores.length > 0 ? (
              <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] font-semibold ${GRADE_COLORS[avgGrade as keyof typeof GRADE_COLORS]}`}>
                <Gauge size={11} />
            ...
            <div className="relative">
              <button onClick={() => setShowCostBreakdown(!showCostBreakdown)}
                ...
                <span className="tabular-nums font-semibold text-base-content/50">
                  ~{apiCalls.costCents < 1 ? `${(apiCalls.costCents * 10).toFixed(0)}‰¢` : `${apiCalls.costCents.toFixed(1)}¢`}
                </span>
              </button>
              {showCostBreakdown && (
                <div className="absolute right-0 top-full mt-1.5 z-50 glass-panel rounded-xl shadow-2xl p-3 min-w-[220px] text-xs animate-scale-in">
                  ...
                  <div className="space-y-1.5 text-[11px]">
                    <div className="flex justify-between"><span className="text-base-content/50">List PRs</span><span className="tabular-nums">{apiCalls.listPR} · ~{(apiCalls.listPR * 1.5).toFixed(1)}K tok</span></div>
                    <div className="flex justify-between"><span className="text-base-content/50">PR details</span><span className="tabular-nums">{apiCalls.getPR} · ~{(apiCalls.getPR * 2).toFixed(0)}K tok</span></div>
                    <div className="flex justify-between"><span className="text-base-content/50">SQL</span><span className="tabular-nums">{apiCalls.sql} · ~{(apiCalls.sql * 0.5).toFixed(1)}K tok</span></div>
                    <div className="border-t border-base-200/50 pt-1.5 mt-1.5 flex justify-between font-bold">
                      <span>Total</span>
                      <span className="tabular-nums text-primary">~{((apiCalls.listPR * 1500 + apiCalls.getPR * 2000 + apiCalls.sql * 500) / 1000).toFixed(1)}K tok · ${(apiCalls.costCents / 100).toFixed(4)}</span>
                    </div>
                  </div>
                </div>
              )}
      ...
      <main className="max-w-3xl mx-auto px-4 py-3 space-y-3">
        ...
        {prs.length === 0 && !activePR && (
          <div className="animate-fade-in flex items-center gap-3 px-4 py-3 rounded-xl border border-success/20 bg-gradient-to-r from-success/5 to-transparent">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[11px] text-base-content/40 leading-tight">0 open PRs · {resolved.length} resolved · {selectedRepos.length} repos monitored</p>
            </div>
            <div className="flex items-center gap-3 shrink-0">
              {selectedRepos.map(r => (
                <a key={r} href={`https://github.com/${r}/pulls`} target="_blank" rel="noopener"
        ...
        {activePR && (
          <>
              ...
              {LIFECYCLE.map((step, i) => {
                ...
                return (
                  <React.Fragment key={step.key}>
                    {i > 0 && <div className={`flex-1 h-px mx-1 ${isDone ? 'bg-success' : isActive ? 'step-active-line h-0.5' : 'bg-base-300'}`} />}
                    <div className="flex flex-col items-center gap-0.5">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs transition-all ${
                        isDone ? 'bg-success/15 text-success ring-1 ring-success/30' :
                        isActive ? 'bg-info/15 text-info ring-2 ring-info/30 animate-glow' :
                        'bg-base-200 text-base-content/20'
                      }`}>
                        {isDone ? <CheckCircle size={11} /> : React.cloneElement(step.icon as React.ReactElement, { size: 11 })}
                      </div>
                      <span className={`text-[10px] font-bold ${isDone ? 'text-success' : isActive ? 'text-info' : 'text-base-content/30'}`}>{step.label}</span>
                    </div>
                  </React.Fragment>
                );
              })}
            ...
            <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border text-xs ${
              nextAction.urgent ? 'bg-warning/8 border-warning/20' : 'bg-base-200/40 border-base-200'
            }`}>
              <ArrowRight size={12} className={nextAction.urgent ? 'text-warning' : 'text-base-content/30'} />
              <span className={`font-semibold ${nextAction.urgent ? 'text-warning' : 'text-base-content/60'}`}>{nextAction.text}</span>
              <span className="text-base-content/30 hidden sm:inline">— {nextAction.detail}</span>
            </div>

            <ActiveSessionCard
              item={activePR} prs={prs} logEntries={activeLogEntries}
              onResolve={resolvePR} onSkip={skipPR} onFail={failPR}
              onCheckStatus={agentCheckStatus} onRePing={agentPingJules}
              actionLoading={actionLoading} agentBusy={agentBusy}
            />
          </>
        )}
        ...
        {!activePR && queued.length > 0 && (
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg border border-warning/20 bg-warning/8">
            ...
            <span className="text-xs font-semibold text-warning flex-1">{queued.length} PR{queued.length > 1 ? 's' : ''} waiting — ready to activate</span>
            <button className="btn btn-xs btn-info gap-1 rounded-lg" onClick={() => agentActivatePR(queued[0])} disabled={agentBusy}>
          ...
          <>
            ...
            {repos.some(r => r.total > 0) && (
              <div className="grid grid-cols-2 gap-2">
                {repos.filter(r => r.total > 0).map(r => {
                  const pct = r.healthPct;
                  return (
                    <div key={r.name} className={`rounded-lg border border-base-200 p-2.5 ring-1 ${
                      pct >= 80 ? 'ring-success/15' : pct >= 50 ? 'ring-warning/15' : 'ring-error/15'
                    }`}>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-1.5">
                          <span className={`text-[10px] font-black w-5 h-5 rounded flex items-center justify-center ${
                            pct >= 80 ? 'bg-success/10 text-success' : pct >= 50 ? 'bg-warning/10 text-warning' : 'bg-error/10 text-error'
                          }`}>{pct >= 80 ? 'A' : pct >= 50 ? 'C' : 'F'}</span>
                          <a href={`https://github.com/${r.name}/pulls`} target="_blank" rel="noopener"
                            className="text-xs font-semibold hover:underline">{r.name.split('/')[1]}</a>
                          <span className="text-[9px] text-base-content/25">{r.total}</span>
                        </div>
                        <span className={`text-sm font-bold tabular-nums ${pct >= 80 ? 'text-success' : pct >= 50 ? 'text-warning' : 'text-error'}`}>{pct}%</span>
                      </div>
                      <div className="flex h-1 rounded-full overflow-hidden bg-base-200/60 gap-px">
                        {r.current > 0 && <div className="bg-success rounded-full" style={{ width: `${(r.current / r.total) * 100}%` }} />}
                        {r.stale > 0 && <div className="bg-warning rounded-full" style={{ width: `${(r.stale / r.total) * 100}%` }} />}
                        {r.dead > 0 && <div className="bg-error rounded-full" style={{ width: `${(r.dead / r.total) * 100}%` }} />}
                      </div>
            ...
            <section>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] text-base-content/20 tabular-nums">{filtered.length}/{prs.length}</span>
                </div>
                <div className="flex gap-1">
                  {([
                    { key: 'all' as const, label: 'All', count: prs.length, dot: '' },
                    { key: 'current' as const, label: 'OK', count: currentCount, dot: 'bg-success' },
              ...
              <div className="rounded-lg border border-base-200 overflow-hidden divide-y divide-base-200/50">
                {filtered.map(pr => {
                  ...
                  return (
                    <div key={`${pr.repo}-${pr.number}`}
                      ...
                      <div className={`absolute left-0 top-1.5 bottom-1.5 w-[2px] rounded-full transition-opacity ${
                        s === 'current' ? 'bg-success' : isDead ? 'bg-error' : 'bg-warning'
                      } opacity-0 group-hover:opacity-100`} />
                      <div className="flex items-center gap-2 text-sm">
                        {health ? (
                          <div className={`flex items-center justify-center w-6 h-6 rounded text-[10px] font-black shrink-0 ${GRADE_COLORS[health.grade]}`}
                            title={`${health.score}/100`}>{health.grade}</div>
                        ) : (
                        ...
                        <a href={pr.url} target="_blank" rel="noopener"
                          ...
                          <span className="font-mono text-[10px] text-base-content/20">#{pr.number}</span>
                          {' '}<span className="text-base-content/70">{pr.title}</span>
                        </a>
                        {qItem ? (
                          <span className={`px-1.5 py-0.5 rounded text-[9px] font-semibold ${
                            qItem.state === 'active' ? 'bg-info/15 text-info' :
                            qItem.state === 'queued' ? 'bg-warning/15 text-warning' :
                            'bg-success/15 text-success'
                          }`}>{qItem.state}</span>
                        ) : s !== 'current' ? (
                          <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="btn btn-xs btn-ghost text-primary rounded h-5 min-h-0 text-[10px]" onClick={() => queuePR(pr)}
                              disabled={actionLoading === `q-${pr.number}`}>Queue</button>
                            {isDead && (
                              <button className="btn btn-xs btn-ghost text-error rounded h-5 min-h-0 text-[10px]" onClick={() => agentAutoClose(pr)}
                                disabled={agentBusy}>Close</button>
                            )}
                          </div>
                        ) : null}
                        <span className="text-[9px] text-base-content/20 tabular-nums shrink-0">{formatAge(pr.createdAt)}</span>
                      </div>
                      {d && (
                        <div className="flex items-center gap-1 mt-1 ml-8 flex-wrap">
                          <span className={`inline-flex items-center gap-0.5 px-1 py-0 rounded text-[9px] font-medium ${
                            d.reviewStatus === 'APPROVED' ? 'bg-success/15 text-success' :
                            d.reviewStatus === 'CHANGES_REQUESTED' ? 'bg-error/15 text-error' :
                            d.reviewStatus === 'PENDING' ? 'bg-warning/15 text-warning' :
                            'bg-base-200 text-base-content/25'
                          }`}>
                            {d.reviewStatus === 'APPROVED' && <><ThumbsUp size={8} /> Approved</>}
                            {d.reviewStatus === 'CHANGES_REQUESTED' && <><ThumbsDown size={8} /> Changes</>}
                            {d.reviewStatus === 'PENDING' && <><Eye size={8} /> Pending</>}
                            {d.reviewStatus === 'NO_REVIEWS' && <><MessageSquare size={8} /> None</>}
                          </span>
                          {d.draft && <span className="px-1 py-0 rounded text-[9px] font-medium bg-base-200 text-base-content/30">Draft</span>}
                          <span className={`inline-flex items-center gap-0.5 px-1 py-0 rounded text-[9px] font-medium ${sizeColor(d.additions, d.deletions)}`}>
                            <FileCode size={8} /> {formatSize(d.additions, d.deletions)}
                          </span>
                          <span className="text-[9px] text-base-content/15 font-mono ml-auto">{pr.repo.split('/')[1]}</span>
                        </div>
                      )}
                    </div>
                  );
                ...
                {filtered.length === 0 && prs.length > 0 && (
                  <div className="py-4 text-center text-xs text-base-content/30">No {filter} PRs</div>
                )}
              </div>
            </section>
          </>
        ...
        <section>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[9px] font-mono text-base-content/25 bg-base-200/60 px-1.5 py-0.5 rounded">{queued.length}</span>
            </div>
            {unqueuedStaleCount > 0 && (
              <button className="btn btn-xs gap-1 btn-outline btn-primary rounded-lg h-6 min-h-0 text-[10px]" onClick={queueAllStale}
          ...
          {queued.length === 0 ? (
            ...
            <div className="space-y-1">
              {queued.map((q, i) => {
                ...
                return (
                  <div key={q.id} className={`queue-item flex items-center gap-2 px-3 py-2 rounded-lg border ${
                    isNext ? 'bg-info/5 border-info/20' : 'bg-base-200/30 border-transparent hover:border-base-300'
                  }`}>
                    <span className="text-base-content/20 font-mono text-[10px] w-3 text-center font-bold tabular-nums">{i + 1}</span>
                    <span className="text-sm">{agent.emoji}</span>
                    <div className="flex-1 min-w-0">
                      <a href={pr?.url || '#'} target="_blank" rel="noopener"
                        className="text-xs font-medium hover:underline truncate block leading-tight">
                        <span className="text-base-content/20 font-mono text-[10px]">#{q.pr_number}</span>{' '}
                        {pr?.title || q.notes || ''}
                      </a>
                      <div className="flex items-center gap-1 mt-0.5">
                        <span className="text-[10px] text-base-content/30 font-mono">{q.repo.split('/')[1]}</span>
                        <span className={`inline-flex items-center gap-0.5 px-1 rounded text-[9px] font-medium border ${pMeta.color}`}>
                          {pMeta.icon} {pMeta.label}
                        </span>
                        {isNext && <span className="text-[9px] font-bold text-info uppercase tracking-wider">next →</span>}
                      </div>
                    </div>
                    <div className="flex items-center gap-0.5 shrink-0">
                      <button className="btn btn-xs btn-ghost btn-square opacity-30 hover:opacity-100 h-5 w-5 min-h-0"
                      ...
                      {canActivate && (
                        <button className="btn btn-xs btn-info gap-0.5 ml-0.5 rounded h-5 min-h-0 text-[10px]"
                          onClick={() => agentActivatePR(q)} disabled={agentBusy}>
                          <Play size={9} /> Go
                        </button>
                      )}
                      <button className="btn btn-xs btn-ghost btn-square opacity-15 hover:opacity-100 hover:text-error h-5 w-5 min-h-0"
          ...
          <section>
            ...
            <div className="rounded-lg border border-base-200 overflow-hidden divide-y divide-base-200/50">
              {activityLog.slice(0, 8).map(entry => (
                <div key={entry.id} className="flex items-center gap-2 py-1.5 px-2.5 text-xs hover:bg-base-200/20 transition-colors">
                  <span className="text-base-content/30 shrink-0">{(ACTION_META[entry.action] || defaultActionMeta).icon}</span>
                  <span className="font-mono text-[9px] text-base-content/15">#{entry.pr_number}</span>
                  <span className="text-[11px] font-medium text-base-content/50">{entry.action}</span>
                  {entry.details && <span className="text-[10px] text-base-content/25 truncate flex-1">— {entry.details}</span>}
                  {!entry.details && <span className="flex-1" />}
                  <span className="text-[9px] text-base-content/15 tabular-nums shrink-0">{formatAge(entry.created_at)}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        {/* ━━━ History ━━━ */}
        {resolved.length > 0 && (
          <HistorySection items={resolved} onRemove={removeFromQueue} onBulkDelete={bulkDelete} onArchive={archiveItem} onBulkArchive={bulkArchive} />
        )}
        ...
        <footer className="text-[9px] text-base-content/15 text-center py-4 flex items-center justify-center gap-1.5">
          ...
          <span className="uppercase tracking-[0.12em] font-medium text-base-content/20">Live · {selectedRepos.length} repos · {resolved.length} resolved</span>
          <span className="w-1 h-1 rounded-full bg-success/30 animate-pulse-soft" />
        </footer>
      </main>
    </div>
...
const ActiveSessionCard: React.FC<{
  item: QueueItem | undefined; prs: PullRequest[]; logEntries: ActivityLogEntry[];
  onResolve: (item: QueueItem, resolution: string) => void;
  onSkip: (item: QueueItem) => void; onFail: (item: QueueItem) => void;
  onCheckStatus: (item: QueueItem) => void; onRePing: (item: QueueItem) => void;
  actionLoading: string | null; agentBusy: boolean;
}> = ({ item, prs, logEntries, onResolve, onSkip, onFail, onCheckStatus, onRePing, actionLoading, agentBusy }) => {
    ...
    <div className="animate-fade-in rounded-xl overflow-hidden border border-info/20 bg-gradient-to-b from-info/5 to-transparent">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] text-base-content/25 font-mono">{item.repo.split('/')[1]}</span>
            </div>
            <a href={pr?.url || '#'} target="_blank" rel="noopener"
      ...
      <div className="px-5 py-4">
        <div className="rounded-lg bg-base-100/60 border border-base-200/80 p-3.5">
          <div className="flex items-center justify-between mb-2.5">
            ...
            <span className="text-[11px] font-bold font-mono text-base-content/30">{item.ping_count} / 3</span>
          </div>
            ...
            {[0, 1, 2].map(i => (
              <div key={i} className="flex-1">
                <div className={`h-2 rounded-full transition-all duration-500 ${
                  i < item.ping_count
                    ? i === 0 ? 'bg-info' : i === 1 ? 'bg-info/60' : 'bg-warning'
                    : 'bg-base-200'
                }`} />
          ...
          {item.first_pinged_at && (
            <div className="flex items-center gap-1.5 mt-3 pt-2.5 border-t border-base-200/60 text-[11px] text-base-content/30">
              ...
              {item.last_pinged_at && item.last_pinged_at !== item.first_pinged_at &&
      ...
      <div className="px-5 pb-4">
        <div className="flex gap-2 flex-wrap">
          <button className="btn btn-sm btn-info gap-1.5 rounded-lg" onClick={() => onCheckStatus(item)} disabled={agentBusy}>
            {agentBusy ? <span className="loading loading-spinner loading-xs" /> : <Search size={13} />}
            Check Status
          </button>
          {item.ping_count < 3 && (
            <button className="btn btn-sm btn-outline btn-info gap-1.5 rounded-lg" onClick={() => onRePing(item)} disabled={agentBusy}>
          ...
          {!resolveOpen ? (
            ...
            <div className="flex gap-1.5 animate-fade-in">
              {([
                { key: 'merged',            label: 'Merged',  cls: 'btn-success', icon: <GitMerge size={11} /> },
                { key: 'rebased',           label: 'Rebased', cls: 'btn-info',    icon: <RotateCcw size={11} /> },
                { key: 'closed',            label: 'Closed',  cls: 'btn-warning', icon: <Archive size={11} /> },
                { key: 'manually-resolved', label: 'Manual',  cls: 'btn-ghost',   icon: <CheckCircle size={11} /> },
              ] as const).map(r => (
                <button key={r.key} className={`btn btn-xs ${r.cls} gap-1 rounded-lg`}
                  onClick={() => { onResolve(item, r.key); setResolveOpen(false); }} disabled={busy}>
                  {r.icon} {r.label}
                </button>
              ))}
              <button className="btn btn-xs btn-ghost rounded-lg" onClick={() => setResolveOpen(false)}>
                <X size={11} />
              </button>
            </div>
          )}
          ...
          <div className="relative pl-5">
            ...
            {logEntries.map((entry, i) => (
              <div key={entry.id} className="relative flex items-start gap-2.5 pb-3 last:pb-0">
                <div className={`absolute left-[-13px] mt-1 w-2 h-2 rounded-full ring-2 ring-base-100 ${
                  i === 0 ? 'bg-info' : 'bg-base-300'
                }`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-base-content/40">{ACTION_META[entry.action]?.icon || <Clock size={13} />}</span>
                  ...
                  {entry.details && (
                    <p className="text-xs text-base-content/35 mt-0.5 ml-[1.375rem]">{entry.details}</p>
                  )}
                </div>
              </div>
            ))}
...
const ActivityHeatMap: React.FC<{ items: QueueItem[] }> = ({ items }) => {
  ...
  const maxCount = Math.max(1, ...Object.values(counts).map((c: { resolved: number; created: number }) => c.resolved + c.created));
  ...
  return (
    <div className="rounded-lg border border-base-200/80 bg-base-100/40 p-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[11px] font-semibold text-base-content/40 uppercase tracking-wider">Session Activity</span>
        <span className="text-[10px] text-base-content/20 ml-auto">{items.length} total sessions</span>
      </div>
      <div className="relative overflow-x-auto">
        {/* Month labels */}
        <div className="flex ml-6 mb-0.5 relative h-3">
          {monthLabels.map((m, i) => (
            <span key={i} className="text-[9px] text-base-content/25 font-medium absolute"
        ...
        <div className="flex gap-[2px]">
          ...
          <div className="flex flex-col gap-[2px] mr-0.5 shrink-0">
            {dayLabels.map((l, i) => (
              <div key={i} className="h-[11px] w-4 text-[8px] text-base-content/20 text-right pr-0.5 leading-[11px]">{l}</div>
            ))}
          </div>
          {/* Grid */}
          {grid.map((week, wi) => (
        ...
        <div className="flex items-center gap-1.5 mt-2 justify-end">
          ...
          {['bg-base-200/50', 'bg-success/25', 'bg-success/45', 'bg-success/65', 'bg-success/90'].map((c, i) => (
      ...
      {tooltip && counts[tooltip.key] && (
        <div className="fixed z-50 bg-neutral text-neutral-content text-[11px] px-2.5 py-1.5 rounded-lg shadow-lg pointer-events-none"
          style={{ left: tooltip.x + 10, top: tooltip.y - 30 }}>
          <div className="font-semibold">{new Date(tooltip.key + 'T12:00:00').toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
...
const HistorySection: React.FC<{
  ...
  const [confirmAction, setConfirmAction] = useState<'delete-selected' | 'delete-all' | 'archive-selected' | null>(null);
  ...
  const filtered = useMemo(() => {
    let list = filterRes === 'all' ? displayItems : displayItems.filter(q => q.resolution === filterRes);
    list = [...list].sort((a, b) => {
      if (sortBy === 'resolved') {
  ...
  return (
    <section className="animate-fade-in">
      <button className="flex items-center gap-2 group cursor-pointer select-none"
        ...
        <ChevronRight size={12} className={`text-base-content/25 transition-transform duration-200 ${
          open ? 'rotate-90' : ''
        }`} />
      </button>
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex gap-0.5 bg-base-200 rounded-lg p-0.5">
              <button
                ...
                onClick={() => { setShowArchived(false); clearSelection(); setFilterRes('all'); }}>
                Active <span className="text-[10px] opacity-50">{nonArchived.length}</span>
              </button>
              <button
                className={`btn btn-xs rounded-md ${showArchived ? 'btn-neutral' : 'btn-ghost text-base-content/50'}`}
                onClick={() => { setShowArchived(true); clearSelection(); setFilterRes('all'); }}>
                <Archive size={11} /> Archived <span className="text-[10px] opacity-50">{archived.length}</span>
              </button>
            </div>
            ...
            {resolutions.length > 1 && (
              <select
                ...
                value={filterRes}
                onChange={e => { setFilterRes(e.target.value); clearSelection(); }}>
            ...
            {selected.size > 0 && (
              <div className="flex gap-1 animate-fade-in">
                {!showArchived && (
                  <button className="btn btn-xs gap-1 btn-outline rounded-lg"
                    onClick={() => setConfirmAction('archive-selected')}>
                    <Archive size={11} /> Archive {selected.size}
                  </button>
                )}
                <button className="btn btn-xs gap-1 btn-outline btn-error rounded-lg"
            ...
            {filtered.length > 0 && selected.size === 0 && (
              <button className="btn btn-xs btn-ghost text-error/50 hover:text-error rounded-lg gap-1"
          ...
          {confirmAction && (
            <div className="rounded-lg border border-error/30 bg-error/5 p-3 flex items-center gap-3 animate-fade-in">
              ...
              <button className="btn btn-xs btn-ghost rounded-lg" onClick={() => setConfirmAction(null)}>Cancel</button>
              <button
          ...
          {filtered.length > 0 && (
            <div className="flex items-center gap-2 px-1">
              <input type="checkbox" className="checkbox checkbox-xs"
                ...
                onChange={toggleAll} />
              <span className="text-[11px] text-base-content/30 cursor-pointer select-none" onClick={toggleAll}>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="rounded-xl border border-base-200 overflow-hidden divide-y divide-base-200/80">
              ...
              {filtered.map(q => {
                ...
                const ageColor = ageDays > 90 ? 'text-error/60' : ageDays > 30 ? 'text-warning/60' : 'text-base-content/25';
                return (
                  <div key={q.id} className={`animate-fade-in flex items-center gap-2.5 px-3 py-2 text-sm transition-colors ${
                    isSelected ? 'bg-primary/5' : 'hover:bg-base-200/30'
                  }`}>
                    <input type="checkbox" className="checkbox checkbox-xs shrink-0"
                      checked={isSelected} onChange={() => toggleSelect(q.id)} />
                    <span className="shrink-0">{q.state === 'skipped' ? '\u23ed\ufe0f' : res.emoji}</span>
                    <span className="font-mono text-[11px] text-base-content/25 w-10 shrink-0">#{q.pr_number}</span>
                    <span className="text-base-content/35 text-xs w-20 truncate shrink-0">{q.repo.split('/')[1]}</span>
                    <span className="flex-1 text-xs text-base-content/45 truncate">{res.label}</span>
                    ...
                    <span className="text-[10px] text-base-content/20 tabular-nums w-10 text-right shrink-0 hidden sm:block" title="Time from queued to resolved">
                      {durationStr(q.created_at, q.resolved_at)}
                    </span>
                    <span className={`text-[10px] tabular-nums w-10 text-right shrink-0 font-medium ${ageColor}`}>
                      {q.created_at ? formatAge(q.created_at) : '\u2014'}
                    </span>
                    <div className="flex gap-0.5 shrink-0">
                      {!showArchived ? (
                        <button className="btn btn-xs btn-ghost btn-square opacity-20 hover:opacity-100"
                          onClick={() => onArchive(q)} title="Archive"
                          aria-label="Archive session">
                          <Archive size={11} />
                        </button>
                      ) : null}
                      <button className="btn btn-xs btn-ghost btn-square opacity-20 hover:opacity-100 hover:text-error"
                        onClick={() => onRemove(q)} title="Delete permanently"
